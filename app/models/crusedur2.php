...
function otentik($username, $password) {
    ...
    try {
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS customer (
            id_cus INT(11) PRIMARY KEY AUTO_INCREMENT,
        ...
        $stmt = $pdo->prepare("SELECT id_cus, username, password, nama FROM customer WHERE username = :username");
        ...
        if ($user) {
            ...
            if (md5($password) === $user['password']) {
                ...
                $_SESSION['user_id'] = $user['id_cus'];
                $_SESSION['user_name_display'] = $user['nama'];
                $_SESSION['user_role'] = 'customer'; // Asumsi semua user di tabel 'admin' adalah Admin
                return true;
            }
        }
    } catch (PDOException $e) {
